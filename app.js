...
function createDefaultState(){
  return { pages: [ createPage("صفحة 1") ], currentPageId: null };
}
...
function setCurrentPage(id){
  ...
  redrawAll();
  saveState();
  // في الجوال: اقفل القائمة بعد اختيار صفحة
  closeSidebar();
}
...
undoBtn.onclick = () => {
  ...
  redrawAll(); saveState(); renderPagesList();
};
  ...
  for(const stroke of page.strokes){
    for(let i=0;i<stroke.points.length;i++){
      ...
      if(stroke.tool === "eraser"){
        ...
        c.lineWidth=(stroke.size*sx)*1.2;
      }else if(stroke.tool==="highlighter"){
        c.globalCompositeOperation="multiply";
        c.strokeStyle=stroke.color;
        c.globalAlpha=0.28;
        c.lineWidth=(stroke.size*sx)*2.2;
      }else{
        c.globalCompositeOperation="source-over";
        c.strokeStyle=stroke.color;
        c.globalAlpha=1;
        c.lineWidth=(stroke.size*sx);
      }
      c.lineCap="round"; c.lineJoin="round";

      if(i===0){
        c.beginPath();
        c.arc(p.x*sx, p.y*sy, Math.max(1,(stroke.size*sx)/2), 0, Math.PI*2);
...
function renderPageToDataURL(page){
  // نرسم صفحة محددة على canvas مخفي بالحجم الأصلي للدفتر
  const off = document.createElement("canvas");
  ...
  // نستخدم نفس منطق الرسم لكن على c بدل ctx
  const drawSeg = (stroke, a, b, isDot) => {
    c.save();

    if(stroke.tool === "eraser"){
      c.globalCompositeOperation = "destination-out";
      c.strokeStyle = "rgba(0,0,0,1)";
      c.lineWidth = stroke.size * 1.2;
      c.lineCap = "round";
      c.lineJoin = "round";
    } else if(stroke.tool === "highlighter"){
      c.globalCompositeOperation = "multiply";
    ...
    c.beginPath();
    if(isDot){
      c.arc(a.x, a.y, Math.max(1, stroke.size/2), 0, Math.PI*2);
      c.fillStyle = c.strokeStyle;
      if(stroke.tool === "highlighter") c.globalAlpha = 0.28;
      c.fill();
    }else{
      c.moveTo(a.x, a.y);
      c.lineTo(b.x, b.y);
...
exportPdfBtn.onclick = () => {
  ...
  // نضيف الصفحات بترتيب: من آخر قائمة لأسفل؟ هنا نخليها من الأقدم للأحدث:
  const pagesInOrder = [...state.pages].reverse();

  pagesInOrder.forEach((p, idx) => {
    const imgData = renderPageToDataURL(p);

    // نحافظ على نسبة الرسم: canvas 900x1200
    const imgW = pageW;
